...
int main()
{
	...
	// main loop
	xTaskCreate(MenuTask, "Menu Task",  configMINIMAL_STACK_SIZE, (void*)0, tskIDLE_PRIORITY, &menuTaskHandle );

	if (menuTaskHandle == NULL) {
	    xil_printf("Failed to create Menu Task\r\n");
	}

	xTaskCreate(DataTask, "Data Task",  configMINIMAL_STACK_SIZE, (void*)0, tskIDLE_PRIORITY, &dataTaskHandle);

	if (dataTaskHandle == NULL) {
	    xil_printf("Failed to create Data Task\r\n");
	}
    xTaskCreate(PIDTask, "PID Task",  configMINIMAL_STACK_SIZE, (void*)0, tskIDLE_PRIORITY, &pidTaskHandle);
	...
	xTaskCreate(ExitTask, "Exit Task",  configMINIMAL_STACK_SIZE, (void*)0, tskIDLE_PRIORITY, &exitTaskHandle);
		if (pidTaskHandle == NULL) {
    ...
    while(1) {
      ...
      //  b = 10; // Example value for b
       // c = 20; // Example value for c
        ...
        //vTaskDelay(1000*configTICK_RATE_HZ /1000); //1 second delay
...
//int localVar;
    ...
    while (1) {
      ...
      int localVar = a;
                    // Example of using the shared variable
                    xil_printf("the value of sharedVariable is %d\r\n", localVar);
        // Release the semaphore
        xSemaphoreGive(dataSemaphore);
    	}
        // Add a small delay to avoid busy-waiting
...
void ExitTask(void *pvParameters) {
    u32 switches, prev_switches = 0xffff;
    volatile bool menuResumed = false;
    while (1) {
        ...
        if (prev_switches != switches && switches != 0) {
            ...
            prev_switches = switches;
                        ...
                        vTaskSuspend(pidTaskHandle);
                        if (menuResumed) {
                            vTaskSuspend(exitTaskHandle); // Suspend the exit task itself only if menu task has been resumed
                        }

                        // Resume menu task
                        vTaskResume(menuTaskHandle);
                        menuResumed = true; // Set the flag to true after resuming the menu task
        }
			...
			if(btns & (btnMsk << i)){
				switch(i){
					...
					case 1:	// btnU - Green duty cycle
							xil_printf("The Value of a now isssssss(add) %d is\r\n",a++);
							break;
					...
					case 4:	// btnD - duty cycle
						xil_printf("The Value of a now isssss(minus) %d is\r\n",a--);
							break;
